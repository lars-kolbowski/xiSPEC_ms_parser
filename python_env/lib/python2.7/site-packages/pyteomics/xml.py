...
class XML(FileReader):
    ...
    def _get_schema_info(self, read_schema=True):
        ...
        try:
            ...
            if not (schema_url.startswith('http://') or
                    schema_url.startswith('https://') or  # LK edit: replace add https support
    ...
    def _handle_param(self, element, **kwargs):
        ...
        if 'unitCvRef' in attribs or 'unitName' in attribs:
            ...
            unit_info = unit_name
        if 'value' in attribs and attribs['value'].strip() != '':
            try:
            ...
            # LK edit start - extract unit info from the cv param
            return_dict['value'] = value
            if kwargs.get('detailed'):
                if unit_info is not None:
                    return_dict['unit'] = unit_info

                if attribs.get('accession'):
                    return_dict['accession'] = attribs.get('accession')

                return {attribs['name']: return_dict}
            else:
                return {attribs['name']: value} # standard output

        else:
            if kwargs.get('detailed'):
                return_dict['name'] = unitstr(attribs['name'], unit_info)
                if attribs.get('accession'):
                    return_dict['accession'] = attribs.get('accession')
            return return_dict
            # LK edit end
...
class IndexedXML(XML):
    ...
    def _build_index(self):
        ...
        self._offset_index = TagSpecificXMLByteIndex(
            self._source, self._indexed_tags, self._indexed_tag_keys)

        self._flat_offset_index = ByteEncodingOrderedDict()

        for key, byte_encoding_ordered_dict in self._offset_index.items():
            self._flat_offset_index.update(byte_encoding_ordered_dict.items())
    ...
    @_keepstate
    def get_by_id(self, elem_id, id_key=None, tag_id=None, **kwargs):
        """
        ...
        """
        try:
            # LK edit to account for change of index to TagSpecific
            # index = self._offset_index

            if tag_id is None:
                index = self._flat_offset_index
            else:
                index = self._offset_index[tag_id]
